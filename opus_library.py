...
parser.add_argument(
    "--src", help="Destination; default is %s" % src, default=src)
parser.add_argument("--dst", help="Source; default is %s" % src, default=dst)
...
src_files = run(["find", args.src, "-type", "f"], encoding="utf8", capture_output=True).stdout.strip().split("\n")

dst_files = run(["find", args.dst, "-type", "f"], encoding="utf8", capture_output=True).stdout.strip().split("\n")

for track in src_files:
    parts = track.replace(args.src, "").split("/")
    f = parts[-1]
    subdirs = "/".join(parts[1:-1])
    pathlib.Path(path.join(args.dst, subdirs)).mkdir(parents=True, exist_ok=True)
...
for track in dst_files:
    if track == "":
        continue
    f = "/".join(track.replace(args.dst, "").split("/")[1:])  # perhaps don't drop leading slash?
    dotparts = f.split(".")
    if dotparts[-1] != "opus":
...
for track in to_encode:
    ...
    safe_run(["ffmpeg","-v", "quiet", "-i", track, "-c:a", "libopus", "-b:a", "128k", track.replace(args.src, args.dst)+".opus"])
